...
are written to 'Replication results/Main case/Elastic net' as Fcast_EN_*/e_EN_*.
"""
...
OUT_DIR = ROOT / "Replication results" / "Main case" / "Elastic net"
...
for h in horizon_vector:
    ...
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_EN_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_EN_h{h}.xlsx")
