"""Random Forest forecast (with SHAP values).
...
2000. Predictors are the seasonal dummies, the cross-country inflation panel, AR
lags, and the global and regional mean factors. The RF is tuned (max_features)
at the first window and re-tuned each December.

Forecasts and errors are written to 'Replication results/Main case/RF'. For
every window the per-observation SHAP values are also saved to
'Replication results/Main case/RF_shapvalues_train_only' (used by the importance and
dependence-plot figures).
"""
...
from Functions import _diff, _lags, _rfTune, _rfPred, _saveModel, load_panels, make_month_dummies

DATA_FILE = ROOT / "Processed data" / "Data_Global.xlsx"
OUT_DIR = ROOT / "Replication results" / "Main case" / "RF"
SHAP_DIR = ROOT / "Replication results" / "Main case" / "RF_shapvalues_train_only"
DATASETS_DIR = ROOT / "Replication results" / "Main case" / "RF_datasets_train_only"
...
for h in horizon_vector:
    ...
    for i in range(ncol):
        ...
        for t in range(win2, len(ret) + 1):
            ...
            yhat_val, rf_model = _rfPred(train, test, 'Returns', best_params, model_config_rf)
            fc[t - 1, i] = yhat_val
            er[t - 1, i] = test['Returns'].iloc[0] - yhat_val

            # Save the training frame (features + Returns) for the dependence-plot
            # figure. Only this RF-with-mean-factors script saves training frames.
            DATASETS_DIR.mkdir(parents=True, exist_ok=True)
            _saveModel(train, test.index[0], str(DATASETS_DIR / "Dataset_rf"), data_temp.name, h)

            # SHAP values over the training window. Only this script saves SHAP.
            SHAP_DIR.mkdir(parents=True, exist_ok=True)
            shap_values = shap.TreeExplainer(rf_model, data=train.drop("Returns", axis=1)).shap_values(
                train.drop("Returns", axis=1), check_additivity=False)
            _saveModel(pd.DataFrame(shap_values, columns=train.drop("Returns", axis=1).columns),
                       test.index[0], str(SHAP_DIR / "Shapvalues_rf"), data_temp.name, h)

    idx, cols = shifted_data['Global'].index, shifted_data['Global'].columns
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_RF_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_RF_h{h}.xlsx")
    print(f"h={h}: wrote RF to {OUT_DIR}")
