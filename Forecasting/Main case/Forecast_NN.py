...
2000. Results are written to 'Replication results/Main case/NN' as
...
OUT_DIR = ROOT / "Replication results" / "Main case" / "NN"
...
for h in horizon_vector:
    ...
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_NN_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_NN_h{h}.xlsx")
