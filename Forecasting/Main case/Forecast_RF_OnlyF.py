"""Random Forest forecast using ONLY the global/regional factors (model F).

One of the two inputs to the forecast-encompassing test (Table 5). Same as
Forecast_RF.py but the cross-country inflation panel is dropped, so the RF sees
only the global and regional mean factors plus the seasonal dummies and AR lags
(the "OnlyF" / factor forecast, F).
...
Fcast_RF_OnlyF_h{h}.xlsx / e_RF_OnlyF_h{h}.xlsx.
"""
...
for h in horizon_vector:
    ...
    for i in range(ncol):
        ...
        for t in range(win2, len(ret) + 1):
            ...
            # Factors only (no inflation panel)
            train_slices = {
                'Ret': train_ret,
    ...
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_RF_OnlyF_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_RF_OnlyF_h{h}.xlsx")
    print(f"h={h}: wrote RF OnlyF (F) to {OUT_DIR}")