...
2000. Results are written to 'Replication results/Main case/XGboost' as
...
OUT_DIR = ROOT / "Replication results" / "Main case" / "XGboost"
...
for h in horizon_vector:
    ...
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_XGB_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_XGB_h{h}.xlsx")
