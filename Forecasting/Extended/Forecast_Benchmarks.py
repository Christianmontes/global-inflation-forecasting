...
'Replication results/Extended' as Fcast_*/e_* Excel files.
"""
...
OUT_BASE = ROOT / "Replication results" / "Extended"
...
for h in horizon_vector:
    ...
    # Save forecasts and errors to the matching Extended sub-folders
    ...
    for m, (sub, fp, ep) in outputs.items():
        ...
        pd.DataFrame(fc[m], index=idx, columns=cols).to_excel(OUT_BASE / sub / f"{fp}_Extended_h{h}.xlsx")
        pd.DataFrame(er[m], index=idx, columns=cols).to_excel(OUT_BASE / sub / f"{ep}_Extended_h{h}.xlsx")
