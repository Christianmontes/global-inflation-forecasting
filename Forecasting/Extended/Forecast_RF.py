...
Forecasts and errors are written to 'Replication results/Extended/RF'.
(This extended-sample variant is forecast-only and does not save SHAP values.)
"""
...
OUT_DIR = ROOT / "Replication results" / "Extended" / "RF"
...
for h in horizon_vector:
    ...
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_RF_Extended_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_RF_Extended_h{h}.xlsx")
