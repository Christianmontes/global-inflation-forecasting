...
2000. Results are written to 'Replication results/Extended/NN' as
...
OUT_DIR = ROOT / "Replication results" / "Extended" / "NN"
...
for h in horizon_vector:
    ...
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_NN_Extended_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_NN_Extended_h{h}.xlsx")
