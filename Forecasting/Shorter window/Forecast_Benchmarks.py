...
10 years of the sample) so forecasts start in 2000. Each model's forecasts and
errors are written to its sub-folder of 'Replication results/Shorter window' as
Fcast_*/e_* Excel files.
"""
...
OUT_BASE = ROOT / "Replication results" / "Shorter window"
...
for h in horizon_vector:
    ...
    # Save forecasts and errors to the matching Shorter-window sub-folders
    ...
    for m, (sub, fp, ep) in outputs.items():
        ...
        pd.DataFrame(fc[m], index=idx, columns=cols).to_excel(OUT_BASE / sub / f"{fp}_ShorterWindow_h{h}.xlsx")
        pd.DataFrame(er[m], index=idx, columns=cols).to_excel(OUT_BASE / sub / f"{ep}_ShorterWindow_h{h}.xlsx")
    print(f"h={h}: wrote RW / SRW / AR dum / CM to {OUT_BASE}")
