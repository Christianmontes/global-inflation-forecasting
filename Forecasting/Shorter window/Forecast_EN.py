...
dummies, AR lags, the global and regional mean factors, and the cross-country
inflation panel; they are standardised before the Elastic Net is tuned (l1_ratio
fixed, alpha by cross-validation) and fit. Results are written to
'Replication results/Shorter window/Elastic net' as Fcast_EN_*/e_EN_*.
"""
...
OUT_DIR = ROOT / "Replication results" / "Shorter window" / "Elastic net"
...
for h in horizon_vector:
    ...
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_EN_ShorterWindow_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_EN_ShorterWindow_h{h}.xlsx")
