...
'Replication results/Shorter window/XGboost' as
...
OUT_DIR = ROOT / "Replication results" / "Shorter window" / "XGboost"
...
for h in horizon_vector:
    ...
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_XGB_ShorterWindow_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_XGB_ShorterWindow_h{h}.xlsx")
