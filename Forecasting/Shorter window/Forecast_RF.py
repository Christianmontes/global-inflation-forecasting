...
10 years of the sample) so forecasts start in 2000. Predictors are the seasonal
dummies, the cross-country inflation panel, AR lags, and the global and regional
mean factors. The RF is tuned (max_features) at the first window and re-tuned
each December.

Forecasts and errors are written to 'Replication results/Shorter window/RF'.
(This shorter-window variant is forecast-only and does not save SHAP values.)
"""
...
OUT_DIR = ROOT / "Replication results" / "Shorter window" / "RF"
...
for h in horizon_vector:
    ...
    pd.DataFrame(fc, index=idx, columns=cols).to_excel(OUT_DIR / f"Fcast_RF_ShorterWindow_h{h}.xlsx")
    pd.DataFrame(er, index=idx, columns=cols).to_excel(OUT_DIR / f"e_RF_ShorterWindow_h{h}.xlsx")
